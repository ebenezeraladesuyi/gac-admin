...
const Gallery = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
          ...
          if (formData.gacImage) {
              formDataToSend.append('gacImage', formData.gacImage); // Correct key should be 'image'
          } else {
      ...
      const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (name === 'gacImage' && files && files.length > 0) {
            setFormData({ ...formData, gacImage: files[0] });
        } else {
            setFormData({ ...formData, gacImage: '' }); // Reset if no file selected
        }
    };
        ...
        <div className="w-[60%] h-full flex flex-col gap-7 items-center p-8">
            ...
            <form  onSubmit={handleSubmit} className=" w-full flex flex-col gap-5">
                ...
                <div className="w-full flex flex-col gap-2">
                    ...
                    name='gacImage' onChange={handleChange}
                    />
                </div>
